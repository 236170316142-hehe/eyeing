...
class ActivityMonitor:
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=5) as response:
                if response.getcode() == 200:
                    ...
                    # Remote update check — fires a background thread if a new version is pending
                    if data.get('update_available') and data.get('update_url') and data.get('update_version'):
                        new_version = str(data['update_version']).strip()
                        if new_version and new_version != self._last_applied_update_version and not self._update_in_progress:
                            self._last_applied_update_version = new_version
                            self._update_in_progress = True
                            t = threading.Thread(
                                target=self._apply_remote_update,
                                args=(str(data['update_url']), new_version),
                                daemon=True
                            )
                            t.start()
    ...
    def _apply_remote_update(self, update_url: str, version: str):
        ...
        try:
            # 1. Download
            tmp_zip = tempfile.mktemp(suffix='.zip', prefix='eyeing_update_')
            req = urllib.request.Request(update_url, headers={'User-Agent': 'EmployeeMonitor/1.0'})
            with urllib.request.urlopen(req, timeout=120) as resp:
                with open(tmp_zip, 'wb') as fout:
                    shutil.copyfileobj(resp, fout)

            # 2. Extract — skip paths that must not be overwritten
            PROTECTED = (
            ...
            self._update_in_progress = False
            self._last_applied_update_version = None  # allow retry on next poll
        finally:
